...
import { PageParams, Router, Store, Task } from "./type.js"
...
export function renderListPage(store:Store,router:Router) {
...
export function renderSettingsPage(store:Store) {
...
export function renderDetailPage({ todoId, store }: PageParams) {
    ...
    console.log(todoId)
    console.log(state)
    let task = state.find((task:Task) => task.id == todoId)
